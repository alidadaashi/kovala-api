...
import shopsJson from './influencers.json';
import Fuse from 'fuse.js';
import { GetShopsDto } from './dto/get-shops.dto';
import { paginate } from 'src/common/pagination/paginate';
import { GetStaffsDto } from './dto/get-staffs.dto';

const shops = plainToClass(Influencer, shopsJson);
const options = {
...
export class ShopsService {
  private shops: Influencer[] = shops;
  ...
  getShops({ search, limit, page }: GetShopsDto) {
    ...
    let data: Influencer[] = this.shops;
    if (search) {
  ...
  getShop(slug: string): Influencer {
